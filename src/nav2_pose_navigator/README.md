...
- `──` = 斜坡边界墙（PGM 障碍物，标记平地和坡面的分界）
- `....` = 斜坡范围（PGM 中为空地，仅 ramp_zone_manager 控制悬挂/限速/锁yaw）
- `═══` = 平台墙（PGM 障碍物）
...
nav2_goal 7.0 2.0 90 --timeout 0        # 不限时（默认）

# 原始 ros2 action 命令（备用）
...
> **坡后目标自动两段**：若目标 X ≥ 9.3，goto_pose_server 会自动先导航到坡前 1m 对齐点（yaw=0，正面朝坡），对齐后再上坡。你只需发最终目标，不需要手动拆成两个点。
...
goal.timeout_sec = 30.0    # 总超时秒数，0 或负数 = 不限时
...
`map_viz_tool.py` 支持离线（纯 A* 预览）和在线（连接 ROS 2 显示 Nav2 真实数据）两种模式。
...
# 在线模式 — 需要 Nav2 运行中，实时显示 /plan + /global_costmap
python3 src/nav2_pose_navigator/nav2_pose_navigator/map_viz_tool.py --team red --online
...
| A* 路径规划 | 点击「A*规划」，A* 算法，自动膨胀障碍物 |
| **Nav2 路径规划** | 点击「Nav2规划」，通过 ComputePathToPose Action 请求 Nav2 规划 |
| **Nav2 实时路径** | 在线模式下自动订阅 `/plan`，青色粗线显示 |
| **全局代价地图** | 在线模式下自动订阅 `/global_costmap/costmap`，半透明叠加（红=障碍，灰=未知） |
| **切换 A*/Nav2** | 点「A*/Nav2」按钮切换：只显示 A* / 只显示 Nav2 / 对比（A* 变淡） |
| **Costmap 开关** | 点「Costmap」按钮切换代价地图叠加层 |
| 切换地图 | 单选按钮切换红方/蓝方/自定义 |
| 预览原点 | 修改「原点X / 原点Y」→ 点「应用原点」，不修改文件 |
| 发送目标到 Nav2 | 点「发送Nav2」，通过 `/go_to_pose` Action 发目标，实时显示剩余距离 |
...
**自动行为**：进坡面 → 升悬挂 + 锁 yaw=0（正面朝坡 +X） + 最低速强制；出坡面 → 降悬挂 + 释放 yaw（Nav2 恢复旋转控制到目标朝向）。外部 `/desired_yaw` 可随时覆盖（优先级高于自动锁）。

### goto_pose_server 参数

**代码位置**：[nav2_pose_navigator/goto_pose_server.py](nav2_pose_navigator/goto_pose_server.py) `__init__` 中的 `declare_parameter`

| 参数 | 默认值 | 说明 |
|------|--------|------|
| `ramp_x_min` | `9.3` | 坡面起始 X (m)，目标 X≥此值时自动插入坡前对齐点 |
| `pre_ramp_offset` | `1.0` | 坡前对齐距离 (m)，对齐点 = (ramp_x_min - offset, goal_y, yaw=0) |

**自动行为**：目标 X ≥ `ramp_x_min` 时自动分两段导航——先到坡前 1m 对齐（yaw=0，正面朝坡），再上坡到最终目标。
...
你发送 GoToPose Action (x, y, yaw)
   ...
   ramp_zone_manager (拦截修改)
         ├── 进坡面 → 升悬挂 + 锁yaw=0 + 限最低速
         ├── 出坡面 → 降悬挂 + 释放yaw (Nav2 恢复旋转)
         ├── 监听 /desired_yaw → 外部 yaw 锁定
...
| 请求 | `target_pose` (PoseStamped, frame_id="map") + `timeout_sec` (float32, 默认 0=不限时) |
...
- `vline(img, y_w, x1, x2, oy, t)` — 水平墙（定 Y 沿 X），t=厚度(mm)，如斜坡边界墙
- `hline(img, x_w, y1, y2, oy, t)` — 垂直墙（定 X 沿 Y），t=厚度(mm)

当前障碍物清单：
- 场地边界墙（武馆顶/擂台底 + 左右侧边）
- 中线（Y=0 分割红蓝）
- 梅花林区（矩形障碍 [3.2,8.0] × 各半场）
- 竞技区高台墙（X=9.45 竖墙）
- **斜坡边界墙**：红 Y=-4.5, 蓝 Y=4.5, X∈[9.3,10.8] — 分隔平地和坡面
...
│   └── GoToPose.action
├── nav2_pose_navigator/
│   ├── goto_pose_server.py        # Action 服务器（坡前对齐 + 两段导航）
│   ├── ramp_zone_manager.py       # 坡面管理（悬挂 + 锁yaw=0 + 限速）
│   ├── odom_to_tf_node.py         # TF 广播（坐标系桥接）
│   ├── cmd_vel_bridge.py          # 速度格式翻译
│   └── map_viz_tool.py            # 地图可视化（离线A* + 在线Nav2路径/代价地图）
├── config/
│   └── nav2_params.yaml           # Nav2 完整参数
├── launch/
│   └── bringup.launch.py          # 一键启动
├── maps/                          # 地图文件 (.pgm + .yaml)
│   └── (由 ../../generate_maps.py 生成)
├── package.xml
├── setup.py
└── setup.cfg
```
