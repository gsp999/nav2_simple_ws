...
- `═══` = 平台墙（PGM 障碍物）
- 地图 PGM 四周外边界会被 `generate_maps.py` 强制画成黑色障碍物
...
service_timeout=90.0        # 最长等生命周期服务出现多久；慢机器主要加这个
transition_timeout=20.0     # 单次 configure/activate 最长等待
...
| `min_cruise_speed` | `1.50` | 离目标较远时，非零线速度会被放大到的最低巡航线速度 (m/s) |
| `enable_cruise_angular_floor` | `true` | 是否启用导航巡航角速度兜底 |
| `min_cruise_angular_speed` | `0.45` | 导航期间，非零角速度会被放大到的最低角速度 (rad/s) |
| `cruise_slowdown_distance` | `0.25` | 距离目标小于该值后只关闭线速度兜底，角速度兜底仍保留 |
...
min_cruise_speed:  1.50
/cmd_vel_adjusted: vx=0.90, vy=1.20  speed=1.50
...
默认情况下，目标点直接发送给 Nav2 做一段导航。若需要在坡后目标时先做坡前对齐（两段导航），可以把 `goto_pose_server.enable_pre_ramp_alignment` 设为 `true`：先在 `(ramp_x_min-0.5, ramp_y_center, yaw=0)` 对齐，再过坡到最终目标。`ramp_zone_manager` 在机器人实际进入 ramp 区时仍会自动抬升底盘。
...
`generate_maps.py` 会自动把 PGM 图像四周 1 像素边界刷成 `WALL=0`，因此地图外边界会被 Nav2 当作静态障碍物。重新生成地图后需要重新 build/source，启动时才会使用安装目录里的新地图：

```bash
colcon build --packages-select nav2_pose_navigator
source install/setup.bash
```
...
> **当前状态**：地图中的梅林区、平台边界墙障碍物已在真实尺寸基础上向外加宽约 0.2m（防碰撞）。梅林区真实 X∈[2.8,7.6]，平台墙真实为 Y=±3.1 单线。生成的地图使用加宽后的尺寸。详见 [generate_maps.py](generate_maps.py) 顶部注释。
...
| `min_cruise_speed` | `declare_parameter` | 1.50 m/s | 导航巡航线速度兜底，觉得直线太猛/太慢改这里 |
| `min_cruise_angular_speed` | `declare_parameter` | 0.45 rad/s | 导航角速度兜底，觉得旋转太慢优先改这里 |
| `cruise_slowdown_distance` | `declare_parameter` | 0.25 m | 离目标多近关闭线速度兜底；不影响角速度兜底 |
...
| `robot_radius` | local/global costmap | 0.20 / 0.30 m | 机器人半径 |
| `inflation_radius` | local/global inflation_layer | 0.25 / 0.30 m | 障碍物膨胀半径 |
| `cost_scaling_factor` | inflation_layer | 6.0 | 膨胀代价衰减，越大衰减越快 |
| `cost_travel_multiplier` | planner_server → GridBased | 2.0 | 全局路径避开高 cost 区域的倾向 |
