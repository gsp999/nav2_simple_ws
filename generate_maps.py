...
Red obstacles (Y negative half, origin [-0.4, -4.6, 0]):
  梅林区(加宽):  X∈[2.7, 7.7], Y∈[-3.5, 0.3]   (真实: X∈[2.8,7.6], Y∈[-3.4,0.2])
  竞技区高台:   X=9.05, Y∈[-3.1, 1.4]

Blue obstacles (Y positive half, origin [-0.4, -1.4, 0]):
  梅林区(加宽):  X∈[2.7, 7.7], Y∈[-0.3, 3.5]   (真实: X∈[2.8,7.6], Y∈[-0.2,3.4])
  竞技区高台:   X=9.05, Y∈[-1.4, 3.1]

抬高平台边界墙 (raised platform edge wall, IN PGM, 加宽为矩形区):
  Red:  X∈[8.9, 10.4], Y∈[-3.2, -3.1]   (真实: Y=-3.1 单线)
  Blue: X∈[8.9, 10.4], Y∈[3.1, 3.2]     (真实: Y=3.1 单线)
...
def generate_blue():
    ...
    rect(img, 2.7, 7.7, -0.3, 3.5, oy)

    # 竞技区高台 wall
    rect(img, 9.05, 9.10, -1.4, 3.1, oy)

    # 抬高平台边界墙 (加宽为矩形: 真实 Y=3.1 单线)
    rect(img, 8.9, 10.4, 3.1, 3.2, oy)
...
def generate_red():
    ...
    rect(img, 2.7, 7.7, -3.5, 0.3, oy)

    # 竞技区高台 wall
    rect(img, 9.05, 9.10, -3.1, 1.4, oy)

    # 抬高平台边界墙 (加宽为矩形: 真实 Y=-3.1 单线)
    rect(img, 8.9, 10.4, -3.2, -3.1, oy)
