...
│       ├── nav2_pose_navigator/
│       │   ├── goto_pose_server.py       # Action 服务器（核心：收到目标→调 Nav2→反馈距离→返回结果）
│       │   ├── ramp_zone_manager.py      # 坡面管理（悬挂升降 + 最低速 + yaw 锁定）
...
- PGM: 240×120 px @ 0.05 m/px。┌─┐=障碍 ═=平台墙 ....=斜坡(非障碍)
...
GoToPose Action (x, y, yaw)
  │
  ▼
goto_pose_server → Nav2 (规划 + MPPI 控制)
  │
  │  /cmd_vel (Twist)
  ▼
ramp_zone_manager   ← 拦截修改速度
  │  监听 /odin1/relocation → 进坡面升悬挂/出坡面降悬挂
  │  监听 /desired_yaw → 锁定朝向（P 控制器覆盖 wz）
  │  监听 /cmd_vel → 坡面最低速强制
  │
...
自动根据位置控制硬件：
- **进坡面** (X∈[9.3,10.8] + Y 在己方坡道范围 [红: -6.0~-4.5, 蓝: 4.5~6.0])：升起悬挂到 75mm，发 targetstate=-1 减速
- **出坡面**：降下悬挂到 30mm，恢复正常
- **坡面最低速**：强制 ≥ 0.25 m/s（防溜车）
- **yaw 锁定**：收到 `/desired_yaw` 后 P 控制器覆盖 Nav2 角速度；发 999.0 解除锁定
...
- **新增**: GoToPose Action 接口，支持运行时动态发目标；`team:=red/blue` 启动参数
